...
function RecipeDetails({ recipe, onClose, isExistingMeal = true }) {
  ...
  const isCustomMeal = recipe.isCustomMeal;
      ...
      <div className="bg-background-light rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            {!isCustomMeal && recipe.dietaryTags && (
        ...
        <div className={`grid grid-cols-1 ${!isCustomMeal ? 'md:grid-cols-2' : ''} gap-6`}>
          {/* Nutritional Information - Always show */}
          ...
          {/* Ingredients - Only show for predefined recipes */}
          {!isCustomMeal && recipe.ingredients && (
            <div className="bg-background rounded-lg p-4">
        ...
        {/* Preparation Time - Only show for predefined recipes */}
        {!isCustomMeal && recipe.prepTime && (
        ...
        {/* Prep Steps - Only show for predefined recipes */}
        {!isCustomMeal && recipe.prepSteps && (
          <div className="mt-6 bg-background rounded-lg p-4">
            <h3 className="text-lg font-semibold text-primary mb-4">Prep Steps</h3>
            <div className="space-y-2">
              {recipe.prepSteps.split('. ').filter(step => step.trim()).map((step, index) => (
